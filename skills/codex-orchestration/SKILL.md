...
Missing or invalid connector state blocks before Codex starts; non-GitHub app
writes keep the global `writes` mode. No automatic approval reviewer intercepts
GitHub writes, so rule-conformant writes are no longer blocked by a
non-deterministic LLM gate (the "Bug 8" observation, now obsolete by removal).
