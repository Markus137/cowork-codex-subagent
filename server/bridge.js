...
function resolveRoleStatePath(environment = process.env) {
  ...
  if (!override) {
    return path.join(os.homedir(), ".config", "cowork-codex-preflight", "roles.json");
  }
  if (
  ...
  async initialize() {
    const result = await this.request(
      "initialize",
      {
        ...
        capabilities: {},
        clientInfo: { name: "cowork-codex-preflight", version: PLUGIN_VERSION },
      },
...
async function createSecureTemporaryDirectory(makeTempDirectory = fs.promises.mkdtemp) {
  const directory = await makeTempDirectory(path.join(os.tmpdir(), "cowork-codex-preflight-"));
